...
class Products extends AdminController
{
    ...
    public function add_product()
    {
        ...
        if (has_permission('products', '', 'view')) {
            ...
            $data['suppliers'] = $this->products_model->get_by_id_supplier();
    ...
    {
        if (!is_admin()) {
            access_denied('Dublicate Product');
        }
        if (!$id) {
            redirect(admin_url('products'));
        }
        ...
        $response = $this->products_model->dublicate_data($id);

       

        if (true == $response) {
            set_alert('success', _l('dublicated', _l('products')));
        } else {
            set_alert('warning', _l('problem_dublicating', _l('products')));
        }
        redirect(admin_url('products'));
    }
    ...
    public function suppliers()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data(module_views_path('products', 'tables/suppliers'));
        }
        $this->load->model(['currencies_model']);
        $data['base_currency'] = $this->currencies_model->get_base_currency();
        ...
        if (has_permission('suppliers', '', 'view')) {
            ...
            if (!empty($post)) {
                ...
                if (false == $this->form_validation->run()) {
                    ...
                    $inserted_id    =$this->products_model->add_suppliers($data);
                    if ($inserted_id) {
                        // handle_product_upload($inserted_id);
                        set_alert('success', 'Supplier Added successfully');
                        redirect(admin_url('products/suppliers'), 'refresh');
                    } else {
                        set_alert('warning', _l('Error Found - Supplier not inserted'));
                    }
                }
            }
            $this->load->model(['currencies_model', 'product_category_model']);
            $data['title']              = _l('add_new', 'supplier');
            $data['action']             = _l('supplier');
            // $data['product_categories'] = $this->product_category_model->get();
            // $data['currencies']         = $this->currencies_model->get();
            // $data['base_currency']      = $this->currencies_model->get_base_currency();

            $this->load->view('products/add_supplier', $data);
        } else {
            access_denied('suppliers');
        }
    }
    ...
    public function edit_supplier($id)
    {
        ...
        close_setup_menu();
        if (has_permission('suppliers', '', 'view')) {
            $original_supplier = $data['suppliers'] = $this->products_model->get_by_id_supplier($id);
            if (empty($original_supplier)) {
                set_alert('danger', _l('not_found_suppliers'));
                redirect(admin_url('products/suppliers'), 'refresh');
            }
            $post=$this->input->post();
            ...
            if (!empty($post)) {
                ...
                if (false == $this->form_validation->run()) {
                    ...
                    $result=$this->products_model->edit_supplier($data, $id);
     ...
     public function delete_supplier($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('products/suppliers'));
        }
        $response = $this->products_model->delete_by_id_supplier($id);
        if (true == $response) {
