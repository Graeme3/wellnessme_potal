...
function appointly_register_menu_items()
{
    ...
    if (staff_can('view', 'appointments') || staff_can('view_own', 'appointments')) {
        ...
        $CI->app_menu->add_sidebar_children_item(APPOINTLY_MODULE_NAME, [
            'slug'     => 'appointly_sidemenu appointly-user-services',
            'name'     => _l('appointment_services'),
            'href'     => admin_url('appointly/appointments/services'),
            'position' => 1,
            'icon'     => 'fa fa-american-sign-language-interpreting',
        ]);

        $CI->app_menu->add_sidebar_children_item(APPOINTLY_MODULE_NAME, [
            'slug'     => 'appointly_sidemenu appointly-user-rooms',
            'name'     => _l('Rooms'),
            'href'     => admin_url('appointly/appointments/rooms'),
            'position' => 2,
            'icon'     => 'fa fa-american-sign-language-interpreting',
        ]);
...
/** 
 * Need to change hours array to string for database
 * Intercepting settings-form
 * @return array
 */
hooks()->add_filter('before_settings_updated', 'modify_appointment_hours');

function modify_appointment_hours($form)
{
