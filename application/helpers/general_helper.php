...
function get_staff_from_service($staffid)
{
    $CI = & get_instance();

    $CI->db->select('firstname')->from(db_prefix() . 'staff')->where('staffid',$staffid);
   
    $firstname = $CI->db->get()->row()->firstname;
...
function get_service_from_room($serviceID)
{
    $CI = & get_instance();

    $CI->db->select('title')->from(db_prefix() . 'service')->where('id',$serviceID);
   
    $title = $CI->db->get()->row()->title;
